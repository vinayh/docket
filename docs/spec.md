...
- **Suggestion dedup**: synthetic `mgn:sug:<hash>` id rotates across `.docx` exports of an unchanged revision, so every re-ingest creates duplicate `canonical_comment` rows. Drop `s.date` from the hash or add a DB UNIQUE on `(version_id, kind, paragraph_index, offset, text)` (`src/domain/comments/suggestions.ts:68`).
...
- **Side panel doesn't re-bind on tab switch**: the SW needs `tabs.onActivated` + `tabs.onUpdated` listeners that push a refetch into the panel (`surfaces/extension/entrypoints/background.ts`).
- **Toolbar icon doesn't toggle the side panel**: clicking the icon while the panel is open is a no-op; track per-tab open-state in SW and close on second click (`surfaces/extension/entrypoints/background.ts`).
- **`bun margin serve` doesn't auto-migrate**: schema drift only surfaces as a `no such column: ...` 500. Call `migrate()` on boot (matches Dockerfile contract) or detect pending migrations and warn loudly (`src/api/server.ts`).
- **Options auto-open on first install missing**: add a `runtime.onInstalled` listener gated on `reason === "install"` that calls `openOptionsPage()` (`surfaces/extension/entrypoints/background.ts`).
- **Reviewer-emails textarea swallows Enter**: controlled-component round-trip filters empty entries on every keystroke, blocking newlines. Store the raw string in form state and `parseEmails` only at save time (`surfaces/extension/entrypoints/sidepanel/views/Settings.tsx:137-148, 208`).
- **Dashboard actions overflow at narrow widths**: per-version `Sync | Comments | Diff | Request review` clips silently. `flex-wrap` as a stopgap; an overflow menu is the proper fix (`surfaces/extension/entrypoints/sidepanel/views/Dashboard.tsx:157-178`).
